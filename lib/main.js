...
var FramesAtom = {
	...
	toggle: function()
	{
		...
		if (!this.validGrammar(this.getGrammar(editor)))
		{
	...
	{
		return editor ? editor.getGrammar().scopeName : '';
	},

	validGrammar: function(grammar)
	{
	...
	previewFile: function(arg)
	{
		...
		// Editor view isn't open, open it first using the same Uri has atom default behaviour
		atom.workspace.open(filePath, {
			searchAllPanes: true
		}).then(function(editor)
		{
			// After open the editor (xvc) open its preview
			that.addPreviewForEditor(editor);
		});
	}
};
