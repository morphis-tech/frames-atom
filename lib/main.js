...
var FramesPreviewView = require('./preview/frames-preview-view');
...
var FramesAtom = {
	activate: function()
	{
		...
		if (parseFloat(atom.getVersion()) < 1.7)
		{
			atom.deserializers.add({
				...
				deserialize: FramesAtom.createFramesPreviewView.bind(FramesAtom)
			});
		}
		...
		return atom.workspace.addOpener(function(uriToOpen)
		{
			...
			if (host === 'editor')
			{
				return that.createFramesPreviewView({
	...
	createFramesPreviewView: function(state)
	{
		if (state.editorId || fs.isFileSync(state.filePath))
		{
			if (FramesPreviewView)
			{
				FramesPreviewView = require('./frames-preview-view');
			}
	...
	toggle: function()
	{
		...
		var grammars = atom.config.get('frames-preview.grammars') || ['text.xml'];
		if (grammars.indexOf(editor.getGrammar().scopeName) < 0)
		{
			console.log('FRAMES: no valid grammars!', editor.getGrammar().scopeName);
			return;
		}
	...
	addPreviewForEditor: function(editor)
	{
		...
		if (atom.config.get('frames.openPreviewInSplitPane'))
		{
			options.split = 'right';
		}
		...
		{
			console.log('FRAMES: view:', framesPreviewView);

			if (isFramesPreviewView(framesPreviewView))
			{
				console.log('FRAMES: activate preview editor!');
