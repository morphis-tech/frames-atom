module.exports = {
	...
	postParsing: function() {
		...
		for (name in ref)
		{
			...
			if (type.xsdChildrenMode === 'extension')
			{
				...
				extenChilds = this.parseExtensionChilds(extenType).filter(Boolean);
				extenAttr = this.parseExtensionAttrs(extenType).filter(Boolean);

				if (extenType.$)
				{
					var linkChildren, linkAttrs;

					linkType = this.types[extenType.$.base];
					if (linkType.xsdChildrenMode == 'extension')
					{
						linkChildren =  this.parseExtensionChilds(linkType.xsdChildren).filter(Boolean);
						linkAttrs = this.parseExtensionAttrs(linkType.xsdAttributes).filter(Boolean);
					}
					type.xsdTypeName = linkType.xsdTypeName;
					type.xsdChildrenMode = linkType.xsdChildrenMode;
					type.xsdChildren = linkChildren ? extenChilds.concat(linkChildren) : extenChilds;
					type.xsdAttributes = linkAttrs ? extenAttr.concat(linkAttrs) : extenAttr;
	...
	parseExtensionChilds: function(extenType)
	{
		...
		{
			n = ref1[i];
			var childs = this.parseChildrenGroups(n.element,'element');
			var choice = n.choice;
			if (choice && choice.length > 0)
			{
				childs = childs.concat(this.parseChildrenGroups(choice[0].element,'element'));
			}
			if (childs)
			{
				for (k = 0, klen = childs.length; k < klen; k++)
				{
					results1.push(childs[k]);
				}
			}
		}
		return results1;
	...
	parseExtensionAttrs: function(extenType)
	{
		var i, len, ref1, results1;
		ref1 = extenType.$$ || [];
		results1 = [];
